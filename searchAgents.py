        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextX][nextY]:
                if (nextX, nextY) in self.corners:
                    visitedCorner[self.corners.index((nextX,nextY))] = 1
                
                nextState = ((nextX, nextY), visitedCorner)
                cost = 1
...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        ...
        for i, element in enumerate(state[1]):
            if element == 0:
                distFromGoals.append(util.manhattanDistance(state[0], corners[i]))

    return max(distFromGoals)
